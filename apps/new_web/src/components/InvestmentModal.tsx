...
const InvestmentModal: React.FC<{
  ...
  onClose: () => void
  onConfirm: () => void
}> = ({ fund, onClose, onConfirm }) => {
  const [amount, setAmount] = useState('')
  const [loading, setLoading] = useState(false)
  const { publicKey, isConnected, connect, signAndSubmitTransaction } =
    useWallet()
  ...
  const handleInvest = async () => {
    ...
    try {
      ...
      // Determine token issuer: prefer consultor's wallet, fallback to payment.destination or fund wallet
      // We compute issuer after creating the order so we can use response data if the fund prop lacks issuer fields
      const { order, payment } = response

      const issuerPublicKey =
        order?.fund?.consultor?.publicKey ||
        payment?.issuerPublicKey ||
        payment?.destination ||
        fund.fundWalletPublicKey
      ...
      try {
        ...
        const existingTrustline = account.balances.find(
          (b: any) =>
            b.asset_code === fundAsset.code &&
            b.asset_issuer === fundAsset.issuer
        )
        ...
        toast.error(
          'Failed to create trustline: ' +
            (trustError.message || 'Unknown error')
        )
        return
      }
      ...
      toast.success(
        `Investment successful! You will receive ${quantity} ${fund.symbol} tokens after approval.`
      )
      onConfirm()
      onClose()
    } catch (error: any) {
